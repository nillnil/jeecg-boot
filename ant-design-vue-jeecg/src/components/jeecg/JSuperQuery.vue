    ...
    <slot>
      <a-tooltip v-if="superQueryFlag" title="已有高级查询条件生效">
        <a-button type="primary" @click="visible=true">
          <a-icon type="appstore" theme="twoTone" :spin="true" />
          <span>高级查询</span>
        </a-button>
      </a-tooltip>
          ...
          <a-col :sm="24" :md="24-5">
            ...
            <a-form v-else layout="inline">
              ...
              <a-form-item label="过滤条件匹配" style="margin-bottom: 12px;">
                <a-select v-model="selectValue" :get-popup-container="node=>node.parentNode">
              ...
              <a-row v-for="(item, index) in queryParamsModel" :key="index" type="flex" style="margin-bottom:10px" :gutter="16">
                ...
                <a-col :span="4">
                  <a-select v-model="item.rule" placeholder="匹配规则" :get-popup-container="node=>node.parentNode">
                ...
                <a-col :span="8">
                  <template v-if="item.dictCode">
                    ...
                    <j-dict-select-tag v-else v-model="item.val" :dict-code="item.dictCode" placeholder="请选择" />
                  </template>
                  <j-select-multi-user
                  ...
                  />
                  <a-select v-else-if="item.options instanceof Array" v-model="item.val" :options="item.options" allow-clear placeholder="请选择" />
                  <j-date v-else-if=" item.type=='date' " v-model="item.val" placeholder="请选择日期" style="width: 100%" />
                  <j-date v-else-if=" item.type=='datetime' " v-model="item.val" placeholder="请选择时间" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" style="width: 100%" />
                  <a-input-number v-else-if=" item.type=='int'||item.type=='number' " v-model="item.val" style="width: 100%" placeholder="请输入数值" />
