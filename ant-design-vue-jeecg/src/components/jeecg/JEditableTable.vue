...
<template>
  <a-spin :spinning="loading">
    ...
    <div :id="`${caseId}inputTable`" class="input-table">
      ...
      <div ref="scrollView" class="scroll-view" :style="{'max-height':maxHeight+'px'}">

        <!-- 渲染主体 body -->
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
          <!-- 扩展高度 -->
          <div class="tr-expand" :style="`height:${getExpandHeight}px; z-index:${loading?'11':'9'};`" />
          <!-- 无数据时显示 -->
          ...
          <draggable :value="rows" handle=".td-ds-icons" @end="handleDragMoveEnd">
            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              >
                ...
                <div v-if="dragSort" class="td td-ds" :style="style.tdLeftDs">
                  <a-dropdown :trigger="['click']" :get-popup-container="getParentContainer">
                      ...
                      >
                        ...
                        >
                          ...
                          <a-select
                            ...
                            :placeholder="replaceProps(col, col.placeholder)"
                            :filter-option="(i,o)=>handleSelectFilterOption(i,o,col)"
                      ...
                      >
                        ...
                        >
                          ...
                          <j-date
                            ...
                            :get-calendar-container="getParentContainer"
                            :placeholder="replaceProps(col, col.placeholder)"
                            :trigger-change="true"
                            :show-time="col.type === formTypes.datetime"
                            :date-format="col.type === formTypes.date? 'YYYY-MM-DD':'YYYY-MM-DD HH:mm:ss'"
...
export default {
  ...
  computed: {
    ...
    tbodyStyle() {
      const style = Object.assign({}, this.style.tbody)
  ...
  watch: {
    ...
    dataSource: {
      ...
      handler: function(newValue) {
        ...
        // 禁用行的id
        const disabledRowIds = (this.disabledRowIds || [])
        newValue.forEach((data, newValueIndex) => {
          ...
          const value = { id: this.caseId + data.id }
          const row = { id: value.id }
          let disabled = false
          this.columns.forEach(column => {
            const inputId = column.key + value.id
            const sourceValue = (data[column.key] == null ? '' : data[column.key]).toString()
            if (column.type === FormTypes.checkbox) {
              // 判断是否设定了customValue（自定义值）
              if (column.customValue instanceof Array) {
                const customValue = (column.customValue[0] || '').toString()
              ...
              if (sourceValue) {
                const fileName = sourceValue.substring(sourceValue.lastIndexOf('/') + 1)
  ...
  mounted() {
    ...
    const { thead, scrollView } = this.$refs
  ...
  methods: {
    ...
    push(record, update = true, rows, insertIndex = null) {
      ...
      const checkboxValues = Object.assign({}, this.checkboxValues)
      const selectValues = Object.assign({}, this.selectValues)
      const jdateValues = Object.assign({}, this.jdateValues)
      const slotValues = Object.assign({}, this.slotValues)
      this.columns.forEach(column => {
        const key = column.key
        const inputId = key + row.id
        // record中是否有该列的值
        const recordHasValue = record[key] != null
    ...
    insert(insertIndex, num = 1) {
      ...
      const newRows = []
      for (let i = 0; i < num; i++) {
        const row = { id: this.generateId(rows) }
    ...
    removeRows(id) {
      ...
      const rows = cloneObject(this.rows)
      ids.forEach(removeId => {
        ...
        if (findAndDelete(rows)) {
          ...
          // 将caseId去除
          const id = this.removeCaseId(removeId)
    ...
    getValuesSync(options = {}) {
      ...
      let error = 0
      const inputValues = cloneObject(this.inputValues)
      const tooltips = Object.assign({}, this.tooltips)
      ...
      for (const value of inputValues) {
        ...
        this.columns.forEach(column => {
          const inputId = column.key + value.id
          if (column.type === FormTypes.checkbox) {
            const checked = this.checkboxValues[inputId]
            if (column.customValue instanceof Array) {
            ...
            const selected = this.selectValues[inputId]
            ...
            const currUploadObj = cloneObject(this.uploadValues[inputId] || null)
            if (currUploadObj) {
          ...
          // 检查表单验证
          if (validate === true) {
            const results = this.validateOneInput(value[column.key], value, column, notPassedIds, false, 'getValues')
    ...
    getValues(callback, validate = true, rowIds) {
      const result = this.getValuesSync({ validate, rowIds })
    ...
    getValuesPromise(validate = true, rowIds) {
      return new Promise((resolve, reject) => {
        const { error, values } = this.getValuesSync({ validate, rowIds })
        if (error === 0) {
          resolve(values)
        } else {
      ...
      values.forEach(item => {
        ...
        for (const newValueKey in newValues) {
          if (newValues.hasOwnProperty(newValueKey)) {
            ...
            const modelKey = `${newValueKey}${this.caseId}${rowKey}`
            // 在 selectValues 中寻找值
    ...
    validateOneInput(value, row, column, notPassedIds, update = false, validType = 'input') {
      const tooltips = Object.assign({}, this.tooltips)
      // let notPassedIds = cloneObject(this.notPassedIds)
      const inputId = column.key + row.id
      ...
      const nextThen = res => {
        ...
        const index = notPassedIds.indexOf(inputId)
        let borderColor = null; let boxShadow = null
        if (!passed) {
    ...
    /** 动态更新表单的值 */
    updateFormValues() {
      const trs = this.el.tbody.getElementsByClassName('tr')
      const trEls = []
      for (const tr of trs) {
        trEls.push(tr)
      }
      ...
      if (this.visibleTrEls.length > 0) {
        ...
        for (const tr of trEls) {
          let isNewest = true
          for (const vtr of this.visibleTrEls) {
            if (vtr.id === tr.id) {
              isNewest = false
              break
            }
    ...
    handleBlurSearch(value, id, row, col) {
      if (col.allowInput === true) {
        ...
        if (typeof value === 'string') {
          ...
          // 翻转删除数组中的项
          for (const index of indexs.reverse()) {
    ...
    handleDragMoveEnd(event) {
      ...
      const diff = Number.parseInt(dataIdx) - oldIndex
      oldIndex += diff
      newIndex += diff
    ...
    showOrHideTooltip(inputId, show, force = false) {
      ...
      const tooltip = this.tooltips[inputId] || {}
    ...
    elemValueChange(type, rowSource, columnSource, value) {
      const column = Object.assign({}, columnSource)
      // 将caseId去除
      const row = Object.assign({}, rowSource)
      row.id = this.removeCaseId(row.id)
      // 获取整行的数据
      const { values } = this.getValuesSync({ validate: false, rowIds: [row.id] })
      if (values.length > 0) {
    ...
    /** 将caseId去除 */
    removeCaseId(id) {
      const remove = id.split(this.caseId)[1]
      return remove || id
    },
    ...
    handleClickDownloadFile(id) {
      const { path } = this.uploadValues[id] || {}
      if (path) {
        const url = window._CONFIG['downloadUrl'] + '/' + path
        window.open(url)
      }
    },
    /** 加载数据字典并合并到 options */
    ...
    buildTdStyle(col) {
      ...
      // checkbox 居中显示
      const isCheckbox = col.type === FormTypes.checkbox
      if (isCheckbox) {
    ...
    /** 预览图片地址 */
    getCellImageView(id) {
      const currUploadObj = this.uploadValues[id] || null
