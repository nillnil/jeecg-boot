      ...
      <a-col :md="18" :sm="24">
        <a-card :bordered="false">
          ...
          <a-input-search
            ...
            @search="onSearch"
          />
          <a-button style="margin-left: 20px" icon="redo" @click="searchReset(1)">重置</a-button>
          <!--用户列表-->
          <a-table
            ref="table"
            ...
            :row-selection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange,type: getType}"
...
export default {
  ...
  methods: {
    initUserNames() {
      ...
      if (this.userIds) {
        const currUserIds = this.userIds
        const userIdsArr = currUserIds.split(',')
